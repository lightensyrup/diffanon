...
class Svc(object):
    def __init__(self, model_path, config_path,
        ...
        self.smodel = self.smodel.to(device)
    ...
    def get_input_features(self, in_path,  refer_path,cr_threshold=0.05):
        ...
        logging.info("timing: load %.3fs", time.time() - t_all)
        selected = False
        ...
        t_step = time.time()
        wav16k_orig = librosa.resample(wav, orig_sr=self.target_sample, target_sr=16000)
        logging.info("timing: resample16k %.3fs", time.time() - t_step)
        wav16k = torch.from_numpy(wav16k_orig).to(self.dev)
        # c = utils.get_hubert_content(self.hubert_model, wav_16k_tensor=wav16k)
        # c = utils.repeat_expand_2d(c.squeeze(0), f0.shape[1])

        t_step = time.time()
        c = self.codec.encode_first(wav16k.unsqueeze(0).unsqueeze(0))
        logging.info("timing: codec_encode %.3fs", time.time() - t_step)
        c = c.to(self.dev)

        t_step = time.time()
        f0 =  self.mpm_model.process_audio(in_path, layer=7,device=self.dev,pitch_shift=False)
        logging.info("timing: mpm_process %.3fs", time.time() - t_step)
        f0 = torch.permute(f0,(1,0))
        t_step = time.time()
        f0 = torch.nn.functional.interpolate(f0.unsqueeze(0),size = c.shape[2],mode="linear")
        logging.info("timing: f0_interpolate %.3fs", time.time() - t_step)
        f0 = f0.to(self.dev)
        ...
        logging.info("timing: mel_spec_init %.3fs", time.time() - t_step)

        if refer_path is not None:
            refer_path = Path(refer_path)
            if refer_path.is_dir():
                t_step = time.time()
                pooled_files = sorted(refer_path.rglob("*.freevcs.pt"))
                if not pooled_files:
                    raise FileNotFoundError(f"No pooled embeddings found under {refer_path}")

                pooled_path = random.choice(pooled_files)
                spk_emb = torch.load(pooled_path, map_location="cpu")
                if isinstance(spk_emb, (list, tuple)) and len(spk_emb) == 1:
                    spk_emb = spk_emb[0]
                spk_emb = torch.as_tensor(spk_emb).float()
                logging.info("timing: load_pooled_emb %.3fs", time.time() - t_step)
                ...
                logging.info("timing: refer_spec_from_in %.3fs", time.time() - t_step)

                t_step = time.time()
                spk_emb = spk_emb.unsqueeze(0).unsqueeze(0)
                spk_emb = spk_emb / spk_emb.norm(p=2)
                spk_emb = spk_emb.repeat(c.shape[2],1,1).permute(1,2,0)
                spk_emb = spk_emb.to(self.dev)
                logging.info("timing: spk_emb_tile %.3fs", time.time() - t_step)
            else:
                t_step = time.time()
                ...
                spec = torch.log(torch.clip(spec, min=1e-7))
                refer = spec.to(self.dev)
                logging.info("timing: refer_spec_from_file %.3fs", time.time() - t_step)
                ...
                ref_wav16k = librosa.resample(ref_wav, orig_sr=self.target_sample, target_sr=16000)
                            #
                ref_wav16k, _ = librosa.effects.trim(ref_wav16k, top_db=20)
                # ref_wav16k = torch.from_numpy(ref_wav16k).to(self.dev)

                spk_emb = self.smodel.embed_utterance(ref_wav16k)
                spk_emb = torch.from_numpy(spk_emb) 
                logging.info("timing: spk_emb_extract %.3fs", time.time() - t_step)
            ...
            spec = torch.log(torch.clip(spec, min=1e-7))
            refer = spec.to(self.dev)
            logging.info("timing: refer_spec_default %.3fs", time.time() - t_step)

            # ref_wav16k = librosa.resample(ref_wav, orig_sr=self.target_sample, target_sr=16000)
                        #
            # wav16k_orig_pro, _ = librosa.effects.trim(wav16k_orig, top_db=20)
            # # ref_wav16k = torch.from_numpy(ref_wav16k).to(self.dev)

            # spk_emb_orig = self.smodel.embed_utterance(wav16k_orig_pro)
            # spk_emb_orig = torch.from_numpy(spk_emb_orig) 
            # #

            # while not selected:
            gaussian_path = Path("exp/data/libritts_all_anon/freevc_gaussian.pt")
            gaussian_scale = 1.0
            gaussian = torch.load(gaussian_path, map_location="cpu")
            mean = gaussian["mean"]
            cov = gaussian["cov"]
            cov_type = gaussian.get("cov_type", "full")
            if cov_type == "diag" or cov.ndim == 1:
                spk_emb = mean + torch.randn_like(mean) * torch.sqrt(cov * gaussian_scale)
            else:
                dist = torch.distributions.MultivariateNormal(
                    mean, covariance_matrix=cov * gaussian_scale
                )
                spk_emb = dist.sample()
            spk_emb = spk_emb.unsqueeze(0).unsqueeze(0)
            spk_emb = spk_emb / spk_emb.norm(p=2)  # unit norm
            # tmp_sim = cosine_similarity(spk_emb_orig,spk_emb.squeeze(0))
        ...
        logging.info("timing: get_input_features total %.3fs", time.time() - t_all)
    ...
    def infer_anon(self,
        ...
        print(f"Feature extraction time: {end - start} ")
        #TODO(anon) the feature extraction for input is get_unit_f0_code, modify it for our features
        with torch.no_grad():
            start = time.time()
