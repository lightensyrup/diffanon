...
class NS2VCDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, codec, all_in_mem: bool = False):
        self.audiopaths = glob(os.path.join(cfg['data']['training_files'], "*/*/*.wav"))
        ...
        self.mpm_model = self.mpm_model.to("cuda")
  
        #NOTE hope length is 320 for us
        # self.codec = codec
    ...
    def get_audio(self, filename):
        ...
        spec = torch.load(filename.replace(".wav", ".wav.st_all.pt")).squeeze(0)

        # f0 = torch.load(filename.replace(".wav",".mpm.pt"))
        f0 = self.mpm_model.process_audio(filename,layer=7,device="cuda",pitch_shift=True)
        # f0, uv = utils.interpolate_f0(f0)
        # f0 = torch.FloatTensor(f0)
        # uv = torch.FloatTensor(uv)

        c = torch.load(filename+ ".st_first.pt").squeeze(0)
        f0 = torch.permute(f0,(1,0))
        f0 = torch.nn.functional.interpolate(f0.unsqueeze(0),size = c.shape[1],mode="linear").squeeze(0)
        # c = utils.repeat_expand_2d(c.squeeze(0), f0.shape[0])

        spk_emb = torch.load(filename.replace(".wav",".freevcs.pt")).unsqueeze(0)
        # spk_emb = torch.nn.functional.normalize(spk_emb)
        if torch.isnan(spk_emb).any():
            print("Spk emb contains Nan!!!")
        spk_emb = spk_emb.repeat(c.shape[1],1).permute(1,0)
        #NOTE repeat_expand just aligns the shapes of f0 and c, maybe we need to do it for ST and MPM
        lmin = min(c.size(-1), spec.size(-1))
        assert abs(c.size(-1) - spec.size(-1)) < 3, (c.size(-1), spec.size(-1), f0.shape, filename)
        assert abs(audio.shape[1]-lmin * self.hop_length) < 3 * self.hop_length
        spec, c, f0 = spec[:, :lmin], c[:, :lmin], f0[:,:lmin]
        spk_emb = spk_emb[:,:lmin]
        audio = audio[:, :lmin * self.hop_length]
        return c.detach(), f0.detach(), spec.detach(), audio.detach(), spk_emb.detach()

    def random_slice(self, c, f0, spec, audio,spk_emb):
        if spec.shape[1] < 30:
        ...
        return refer,c, f0, spec, audio, spk_emb
...
class TextAudioCollate:
    ...
    def __call__(self, batch):
        ...
        f0_dim = batch[0][2].shape[0]
        spk_dim = batch[0][5].shape[0]
        c_padded = torch.FloatTensor(len(batch), contentvec_dim, max_c_len+1)
        f0_padded = torch.FloatTensor(len(batch), f0_dim, max_c_len+1)
        ...
        spk_emb_padded = torch.FloatTensor(len(batch),spk_dim,max_c_len+1)
        ...
        for i in range(len(ids_sorted_decreasing)):
            ...
            spk_emb_padded[i,:,:len_contentvec] = row[5][:] 
            # uv_padded[i, :len_contentvec] = row[5][:]

        return c_padded, refer_padded, f0_padded, spec_padded, wav_padded, spk_emb_padded, lengths, refer_lengths 
